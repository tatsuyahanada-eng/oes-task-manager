...
- Apache / nginx は「来たアクセスを `localhost:3000` へ転送する」設定だけ行います（後述 7-3）
...
`DBC_HOST` を `127.0.0.1` 以外にすると、認証情報が無い限りサーバは起動を拒否します。
...
export PORT=3000
export DBC_HOST=127.0.0.1                       # リバースプロキシ経由にするので localhost のまま
export DBC_AUTH_USER='dbadmin'
export DBC_AUTH_PASS='ここに生成したパスワード'   # 16文字以上が必須
export DBC_MASTER_KEY='ここに別途生成した文字列'   # 接続情報の暗号鍵 (後述)
...
> リバースプロキシ (nginx / Apache) を前段に置く構成では、`DBC_HOST` は `127.0.0.1` のままにします。
> プロキシを使わず直接公開する場合のみ `DBC_HOST=0.0.0.0` にしてください。
...
### 7-4. アクセス元の制限 (できるなら必ず)

固定 IP から使うなら、7-3 の nginx 設定の `allow` / `deny` を有効にしてください。
**認証より確実に効きます。**
...
| ブラウザで認証ダイアログが出続ける | パスワードの打ち間違いか、プロキシが `Authorization` ヘッダを落としています。7-3 の注記を参照 |
