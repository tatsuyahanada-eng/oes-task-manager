...
**トップページはログイン画面です。** ログインしないと、画面も API も一切見られません。
...
> **この初期パスワードのままでは、localhost 以外での待ち受けができません。**
> サーバは起動時にこれを検査し、初期パスワードのまま外部公開しようとすると起動を拒否します。
> ログイン後、設定画面から必ず変更してください。

パスワードは **scrypt でハッシュ化**して `data/auth.json` に保存されます。平文はどこにも残りません。

| 項目 | 内容 |
|---|---|
| セッション | httpOnly / SameSite=Strict の Cookie。JavaScript からは読めません |
...
| サービス | 可否 | 理由 |
|---|---|---|
| **さくらの VPS** | ○ | Linux + root 権限。Node.js を自分で導入でき、常駐も自由。**推奨** |
| **さくらのクラウド** | ○ | 同上 |
| **さくらのレンタルサーバ** | **✗ 使えません** | 共用サーバのため**常駐プロセスの実行が禁止**されている (公式)。<br>Node.js の導入自体はスタンダード以上で可能だがサポート対象外。<br>加えて OS が FreeBSD で、Oracle ドライバの同梱バイナリ (Linux/macOS/Windows) が使えない |
| **さくらのマネージドサーバ** | △ 要確認 | FreeBSD のため Oracle ドライバは Thin モードのみ。常駐可否は契約条件を確認 |
| **ConoHa VPS / Xserver VPS / AWS Lightsail** | ○ | Linux + root 権限 |
| **PHP のみの共用レンタルサーバ全般** | ✗ | Node.js が動かない |

> **共用レンタルサーバ (さくらのレンタルサーバを含む) では動きません。**
> このツールは Node.js のプロセスが常時起動し続ける必要がありますが、
> 共用サーバではこれが規約上・技術上できないためです。
> **VPS を使うか、手元の PC で動かしてください** (移行作業だけなら手元の PC で十分です)。
...
|---|---|---|
| **full** (`node_modules` 同梱) | `db-controller-full.tar.gz` | `db-controller-full.zip` |
| **src** (ソースのみ) | `db-controller-src.tar.gz` | `db-controller-src.zip` |
...
> **迷ったら `db-controller-full.tar.gz`** (FTP なら `-full.zip`) を選んでください。
> src 版はサーバが npm レジストリに到達できないと `npm install` が失敗します。

---

## 4. index.html はどこに置くの？

よくある誤解なので先に説明します。

**このアプリは、レンタルサーバに index.html を置く形ではありません。**

### 普通のホームページ / PHP サイトの場合

```
public_html/index.html  ──→  Apache が直接ファイルを返す
```

`public_html`（公開ディレクトリ）に置いたファイルが、そのまま URL で読まれます。
...
ブラウザ
   │
   ▼
Apache / nginx  （リバースプロキシ：転送するだけ）
   │
   ▼
node server.js  （このアプリ本体。ポート 3000 で待ち受け）
   │
   ▼
public/index.html を返す
```

`public/index.html` は確かに存在しますし、**トップページはこの index.html です。**
ただし、それを返すのは Apache ではなく **`server.js`（Node.js）** です。

そのため、

- **`public_html` には何も置きません**
- アプリ一式は公開ディレクトリの**外**（例 `~/apps/db-controller`）に置きます
- Apache / nginx は「来たアクセスを `localhost:3000` へ転送する」設定だけ行います（後述 8-3）

### `public/` だけを public_html に置くのはダメなの？

**動きません。** 画面は `/api/...` を呼んでデータを取りますが、それに答えられるのは
Node.js のプロセスだけです。静的ファイルとして置いても、画面は出るがデータが一切表示されません。

そして**アプリ一式を `public_html` に置くのは危険**です。次で説明します。
...
```
https://example.com/db-controller/data/connections.json   ← 誰でも取得できてしまう
```

```
✗ /home/ユーザー名/public_html/db-controller     ← 危険
✗ /var/www/html/db-controller                    ← 危険
✓ /home/ユーザー名/apps/db-controller            ← 安全
```
...
| サーバの種類 | 配置ディレクトリ | 補足 |
|---|---|---|
| **VPS / クラウド** (さくら VPS、ConoHa、Xserver VPS など) | `/home/ユーザー名/apps/db-controller` | root ではなく一般ユーザーで動かす |
| **VPS (システム全体で運用)** | `/opt/db-controller` | systemd で常駐させる場合。専用ユーザーを作ると安全 |
| **Node.js 対応の共有サーバ** (mixhost など) | 管理画面の「Node.js アプリ」で指定された<br>アプリケーションルート (公開ディレクトリの外) | 管理画面側で起動ファイルに `server.js` を指定 |
| **手元の PC** | どこでも可 (例: `~/db-controller`) | 移行作業だけならこれで十分 |

**ディレクトリ名は `db-controller` のままにしてください** (アーカイブを展開するとこの名前で作られます)。
変更しても動きますが、DEPLOY.md 中のコマンド例と食い違います。

### 展開後の中身

```
db-controller/          ← このディレクトリごと配置する
...
scp db-controller-full.tar.gz ユーザー名@サーバ:~/
...
mkdir -p ~/apps
cd ~/apps
tar xzf ~/db-controller-full.tar.gz
#   → ~/apps/db-controller/ ができる
...
1. `db-controller-full.zip` を**そのまま**アップロード
   （置き場所は公開ディレクトリの**外**。管理画面上では `public_html` と同じ階層にある領域）
2. ファイルマネージャで zip を右クリック →「解凍 / 展開」
3. `db-controller/` ができたら、zip ファイルは削除
4. `data` ディレクトリのパーミッションを `700` に変更

**B-2. 展開機能が無い場合**

手元の PC で zip を展開してから、できた `db-controller` フォルダを丸ごと転送します。
...
初期パスワードのままでは、`DBC_HOST` を localhost 以外にすると**起動を拒否**します。

1. まず localhost で起動する（`npm start`）
2. ブラウザで <http://127.0.0.1:3000> を開き、`welsysadm` / `Welsys@1234` でログイン
3. 「設定」→「パスワードを変更」で、10 文字以上・英大小文字と数字を含むものに変更
4. サーバを止めて、公開用の設定で起動し直す

環境変数で最初から指定することもできます（`data/auth.json` が無い状態で起動したときのみ有効）。

```bash
# パスワードを生成する
node -e "console.log(require('crypto').randomBytes(18).toString('base64url'))"

# data/auth.json を作り直す
rm -f data/auth.json
DBC_AUTH_USER='welsysadm' DBC_AUTH_PASS='ここに新しいパスワード' npm start
```
...
export PORT=3000
export DBC_HOST=127.0.0.1                        # リバースプロキシ経由なので localhost のまま
export DBC_MASTER_KEY='ここに暗号鍵'              # 接続情報の暗号鍵（後述）
# export DBC_SESSION_IDLE_MS=1800000             # 無操作でのログアウト（既定 30 分）
...
> パスワードは `data/auth.json`（ハッシュ）で管理するため、`start.sh` に書く必要はありません。
...
### 8-4. アクセス元の制限 (できるなら必ず)

固定 IP から使うなら、8-3 の nginx 設定の `allow` / `deny` を有効にしてください。
**認証より確実に効きます。**
...
`/etc/systemd/system/db-controller.service`
...
Type=simple
User=あなたのユーザー名
WorkingDirectory=/home/あなたのユーザー名/apps/db-controller
ExecStart=/usr/bin/node server.js
Restart=on-failure
RestartSec=5

Environment=PORT=3000
Environment=DBC_HOST=127.0.0.1
Environment=DBC_AUTH_USER=dbadmin
Environment=DBC_AUTH_PASS=ここにパスワード
Environment=DBC_MASTER_KEY=ここに暗号鍵

# 環境変数にパスワードを直接書きたくない場合は EnvironmentFile を使う
# EnvironmentFile=/home/ユーザー名/apps/db-controller/.env
...
```bash
sudo chmod 600 /etc/systemd/system/db-controller.service
sudo systemctl daemon-reload
sudo systemctl enable --now db-controller
sudo systemctl status db-controller
journalctl -u db-controller -f      # ログを見る
```
...
```bash
npm install -g pm2
pm2 start start.sh --name db-controller
pm2 save
pm2 startup          # 表示されたコマンドを実行すると自動起動になります
pm2 logs db-controller
```
...
`DBC_MASTER_KEY` を環境変数で明示しておくと、`data/.masterkey` に依存せず設定を移せます。
...
この値を `DBC_MASTER_KEY` に設定しておけば、`data/connections.json` だけをコピーすれば移行できます。
...
- [ ] `DBC_MASTER_KEY` を控えた
- [ ] PWA としてインストールしたい場合、HTTPS でアクセスできる（localhost 以外では HTTPS が必須）
...
| 症状 | 原因と対処 |
|---|---|
| `起動を中止しました` と出て起動しない | `DBC_HOST` が localhost 以外なのに `DBC_AUTH_USER` / `DBC_AUTH_PASS` が未設定。表示されたメッセージのとおり設定してください |
| `ポート 3000 は既に使用されています` | 別のプロセスが使用中。`PORT=3001` など別のポートを指定してください |
| ブラウザで認証ダイアログが出続ける | パスワードの打ち間違いか、プロキシが `Authorization` ヘッダを落としています。8-3 の注記を参照 |
...
| 接続情報を開くとパスワードの復号エラー | `DBC_MASTER_KEY` または `data/.masterkey` が変わっています。元の鍵に戻すか、接続情報を登録し直してください |
